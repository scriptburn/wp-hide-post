...
class TestCustomPosts extends WPBASE_UnitTestCase
{
...
// Add category metabox to page
        ...
        add_filter('pre_get_posts', function ($query)
        {
            if (is_category() || is_tag() || is_author() || is_archive() || is_feed())
            {
                ...
                if (!$post_type)
                {
                    ...
                    $post_type = array('post', 'page', 'acme_product');
    ...
    {
        $this->create_posts_if_not_created('acme_product');
    ...
    public function hidden_on_post_tag($index, $hidden = true)
    {
        ...
        $hidden   = (int) $hidden;
        $showHide = "_" . $hidden;
//$post_type, $taxonomy, $fields = 'all', $args
        // p_l($this->posts[$index . $showHide]);
        // $term=$this->get_terms_by_post_type('tag','acme_product','all',);
        //  p_d($term);
        $term = get_term_by('name', $this->posts[$index . $showHide]['tags_input'][0], 'post_tag');
        //$terms=get_post_taxonomies($this->posts[$index . $showHide]['ID']);
        //p_d($term);
        //exit();
    ...
    public function hidden_on_rel_link($index, $hidden = true)
    {
        ...
        $posts = $this->create_posts_if_not_created('acme_product');
