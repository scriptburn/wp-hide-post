...
class TestPages extends WPBASE_UnitTestCase
{
    use SCBExec;
...
// Add category metabox to page
        ...
        {
            if (is_category() || is_tag() || is_author() || is_archive() || is_feed())
            {
    ...
    {
        $this->create_posts_if_not_created('page');
    ...
    public function hidden_on_post_category($index, $hidden = true)
    {
        ...
        //     $ret = $this->copy_current_db_to('scriptbu_wphp_mirror');
    ...
    public function hidden_on_post_author($index, $hidden = true)
    {
        ...
        $hidden   = (int) $hidden;
        $showHide = "_" . $hidden;
        p_l(get_author_posts_url($this->currentUser));
    ...
    public function hidden_on_rel_link($index, $hidden = true)
    {
        ...
        $posts = $this->create_posts_if_not_created('page');
    ...
    public function test_hidden_on_post_front()
    {
        ...
        $posts     = $this->hidden_on_post_front('hide_frontpage', true);
        $cat_posts = $this->hidden_on_post_category('hide_frontpage', true);

        $tags_posts         = $this->hidden_on_post_tag('hide_frontpage', true);
        $author_posts       = $this->hidden_on_post_author('hide_frontpage', true);
        $archive_posts      = $this->hidden_on_post_archive('hide_frontpage', true);
        $search_posts       = $this->hidden_on_post_search('hide_frontpage', true);
        $feed_posts         = $this->hidden_on_post_feed('hide_frontpage', true);
        $searchengine_posts = $this->hidden_on_search_engine('hide_frontpage', true);
    ...
    public function test_hidden_everywhere_no_search_engine()
    {
        $index     = 'hide_always';
        ...
        $this->assertNotContains($posts[0], $posts[1]['nonstatic'], 'Must be hidden on the front page(nonstatic)');
        $this->assertNotContains($posts[0], $posts[1]['static'], 'Must be hidden  on the front page(static)');
        $this->assertNotContains($cat_posts[0], $cat_posts[1], 'Must be hidden on the category page');
    ...
    public function test_hidden_everywhere_no_search_engine_and_search()
    {
        $index     = 'nohide_search';
        $posts     = $this->hidden_on_post_front($index, true);
        $cat_posts = $this->hidden_on_post_category($index, true);
    ...
    public function test_hidden_search_engine()
    {
        $index = 'no_index';
        $posts = $this->hidden_on_post_front($index, true);
