...
function wphp_ispro($sup = true)
{
	...
	if (is_null($pro))
	{
		$pro = defined('WPHP_PRO') &&
			WPHP_PRO;
		$license = false;
		if ($pro)
		{
			$license = function_exists('scb_get_license');
		}
		if (!$license)
		{
			$pro = false;
		...
		{
			$license = scb_get_license('wp-hide-post-pro');
			$pro = is_object($license) &&
			method_exists($license, 'is_valid') && $license->is_valid();
		}
